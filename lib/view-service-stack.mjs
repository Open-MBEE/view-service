...
import {Choice, Condition, Map, Pass, Result, StateMachine, Succeed, TaskInput,} from '@aws-cdk/aws-stepfunctions';
...
export class ViewServiceStack extends Stack {
    ...
    constructor(scope, id, props) {
        ...
        const confluenceSecretKey = scope.node.tryGetContext('confluence_secret_key');
        const confluenceSecretArn = scope.node.tryGetContext('confluence_secret_arn');
        const confluenceToken = Secret.fromSecretAttributes(this, confluenceSecretKey, {
            secretArn: confluenceSecretArn
        ...
        const exportConfluenceLambda = new NodejsFunction(this, 'ExportConfluence', {
            ...
            environment: {
                'CONFLUENCE_TOKEN': confluenceToken.secretValue.toString(),
        ...
        const createCloudWatchEvent = new Rule(this, 'ViewServiceCloudWatchEvent', {
            schedule: Schedule.cron({minute: '0'}),
            targets: [
                new SfnStateMachine(viewServiceStateMachine, {role: stateMachineEventRole})
            ]
        });
    }
}
