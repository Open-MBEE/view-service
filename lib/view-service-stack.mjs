...
export class ViewServiceStack extends Stack {
    ...
    constructor(scope, id, props) {
        ...
        const checkLoadStatusLambda = new NodejsFunction(this, 'CheckLoadStatus', {
            ...commonNodejsFunctionProps,
            handler: 'checkLoadStatus',
            environment: {
                'SPARQL_ENDPOINT': `${resolvedEndpoint}`,
                'NEPTUNE_S3_BUCKET_URL': `s3://${neptuneStagingBucket.bucketName}`,
                'NEPTUNE_S3_IAM_ROLE_ARN': neptuneLoadingRole.roleArn,
        ...
        const checkLoadStatusJob = new LambdaInvoke(this, 'CheckLoadStatusJob', {
            resultPath: '$.impl.neptune.load',
            payload: TaskInput.fromObject({
                // TODO Review IRI definition
                'graph.$': "States.Format('https://wiki.jpl.nasa.gov/display/{}', $.impl.space)",
                'prefix.$': '$.requestId',
                'jobId.$': '$.impl.neptune.jobId'
            }),
            payloadResponseOnly: true,
            lambdaFunction: checkLoadStatusLambda,
        });
        ...
        const finish = new Choice(this, 'JobCompleteCheck')
            .when(Condition.isPresent('$.impl.neptune.jobId'), checkLoadStatusJob.next(
                new Choice(this, 'JobStatusCheck')
                    .when(Condition.stringEquals('$.impl.neptune.load.0.payload.overallStatus.status', 'LOAD_IN_PROGRESS'), checkLoadStatusJob)
                    .otherwise(new Pass(this, 'CheckComplete'))
            ))
            .when(Condition.stringEquals('$.impl.neptune.load.0.payload.overallStatus.status', 'LOAD_COMPLETED'), new LambdaInvoke(this, 'SuccessNotification', {
